/* This program reads in a text file to populate a
database of employees. Then it prompts the user for actions
to take on the database. A custom helper library is built to 
...
This function will return only one employee and it is the first one listed in the DB via 
Employee ID. If the user is not found it returns a "User not found" message */
